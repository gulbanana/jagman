...
describe('opencode server lifecycle', () => {
	describe('server startup', () => {
		it('spawns opencode with shell:true and correct arguments', async () => {
			...
			await agent.loadSessions(['/test/repo'], 10);
		...
		it('reuses the server across sequential loadSessions calls', async () => {
			spawnEmitsUrl();
			const agent = new OpenCodeAgent();
			await agent.loadSessions(['/repo/a'], 10);
			await agent.loadSessions(['/repo/b'], 10);
	...
	describe('server startup errors', () => {
		it('rejects when spawn emits an error', async () => {
			...
			const agent = new OpenCodeAgent();
			await expect(agent.loadSessions(['/test/repo'], 10)).rejects.toThrow(
		...
		it('rejects on timeout when the server never reports its URL', async () => {
			...
			const agent = new OpenCodeAgent();
			const promise = agent.loadSessions(['/test/repo'], 10);
